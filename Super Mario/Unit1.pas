...
begin
  if not EnumDisplaySettings(nil, ENUM_REGISTRY_SETTINGS, FLastMode) then begin
    ShowMessage('Não consegui pegar as configurações atuais do vídeo. Não será possível mudar de modo');
  end else begin
    ...
    if ChangeDisplaySettings(D, CDS_FULLSCREEN) <> DISP_CHANGE_SUCCESSFUL then begin
      ShowMessage('Não foi possível mudar o modo de vídeo');
    end;
  end;
end;
var i: Integer;
...
begin
  ShowMessage('Vagner Matheus Games, agradece o apoio!');
  ChangeMode2;
end;
