...
begin
  // PONTOS DA COLISÃO PELA DIREITA
  ...
  // PONTOS DA COLISÃO PELA ESQUERDA
  A.x:= x div (1 shl POT);
  A.y:= y div (1 shl POT);
...
begin
  if not EnumDisplaySettings(nil, ENUM_REGISTRY_SETTINGS, FLastMode) then begin
    ShowMessage('Não consegui pegar as configurações atuais do vídeo. Não vou mudar de modo');
  end else begin
    ...
    if ChangeDisplaySettings(D, CDS_FULLSCREEN) <> DISP_CHANGE_SUCCESSFUL then begin
      ShowMessage('Não consegui mudar o modo de vídeo...');
    end;
  end;
end;
var i: Integer;
